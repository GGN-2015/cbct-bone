...
from .models import ModelManager, ProgressCallback

SegmentationBackend = Literal["v4", "adaptive"]
...
class CBCTBoneExtractor:
    ...
    def __init__(
        ...
        *,
        ...
        self.segmenter = segmenter if segmenter is not None else V4CBCTBoneSegmenter()
...
def segment(
    ...
    *,
    ...
    backend: SegmentationBackend = "v4",
    ...
    if backend not in ("v4", "adaptive"):
        raise ValueError("backend must be 'v4' or 'adaptive'")
    if backend == "v4":
        if config is not None:
            raise ValueError("config applies only to the adaptive backend")
        manager = ModelManager(cache_dir=cache_dir) if model_path is None else None
        segmenter: BoneMaskSegmenter = V4CBCTBoneSegmenter(
            neural_config,
        ...
        if neural_config is not None or model_path is not None or offline or providers:
            raise ValueError("neural model options require backend='v4'")
